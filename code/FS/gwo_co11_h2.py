...
from FS.functionHO2 import error_rate
import math
import random
...
def init_position(lb, ub, N, dim):
    ...
    Xm = np.zeros([N, int(dim * 0.4)], dtype='float')
    Xl = np.zeros([N, (dim - (int(dim * 0.05) + int(dim * 0.4)))], dtype='float')
    for i in range(N):
        random.seed()
        ...
        sample_list_m_1 = [i for i in range(int(dim * 0.4))]
        ...
        sample_list_l = [i for i in range((dim - (int(dim * 0.05) + int(dim * 0.4))))]
        ...
        for i in range(3):
            ...
            for d in range(dim):
                if Xbin[0, d] != 0:
                    B[i, d] = 0
                # else:
                #     B[i, d] = Xalpha[0, d] + random.uniform(-(t / max_iter), 0)
